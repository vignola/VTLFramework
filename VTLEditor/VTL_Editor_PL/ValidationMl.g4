...
start
  :
  statement* EOF; //-> ^(RULE_TAG statement*);

/* Assignment */

statement
  :
  (variableRef ASSIGN expr)   // ->      ^(ASSIGN_TAG variableRef expr)  
  ;
...
exprAtom
  ...
  | (MIN '(' b1=expr ')'      /*->        ^(DATASET_MIN_TAG $b1)*/
  )
  | (MAX '(' b1=expr ')'
      //->
      //  ^(DATASET_MAX_TAG $b1)
  ...
  | (POWER '(' b1=expr ',' b5=powerExponent ')'
      //->
