package com.capgemini.istat.vtlcompiler.vtlsemantic.expression.persist;
...
public class PersistValidation implements OperatorValidation {
    ...
    public LinkedList<ResultExpression> resolve(Map<KeyVariables, Object> variables) throws Exception {
        ...
        result.addAll(semanticFactory.checkSemantic(vtlPersist.getVtlExpression(), variables));
        String datasetName = vtlPersist.getVtlVarId().getName();
        result.addFirst(
                functionResultService.assignDatasetName(
                        result.getFirst().getResult(),
                        datasetName,
                        Operator.PERSIST
                )
        );
        result.getFirst().setCommand(vtlPersist.getCommand());
        return result;
    }
}
