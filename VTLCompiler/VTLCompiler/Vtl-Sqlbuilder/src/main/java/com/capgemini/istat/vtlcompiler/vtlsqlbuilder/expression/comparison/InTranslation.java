...
@Component
@Scope("prototype")
public class InTranslation implements OperatorTranslation {
    ...
    public void setVtlExpression(VtlExpression vtlExpression) {
        this.vtlInExpression = (VtlInExpression) vtlExpression;
    }
    ...
    public SqlResult translate(Map<KeyVariables, Object> variables) throws Exception {
        SqlResult sqlResult = translationFactory.translate(vtlInExpression.getVtlElement(), variables);
        Boolean isWhereCondition = (Boolean) variables.getOrDefault(KeyVariables.FILTER_CLAUSE, false);
        if (isWhereCondition) {
            sqlResult = sqlResultService.applyInNotInCondition(vtlInExpression, sqlResult);   
        } else {
            sqlResult = sqlResultService.applyInNotInFunction(vtlInExpression, sqlResult);
        }
        return sqlResult;
    }    
}