 ...
 * La classe offre dei metodi specifici per PostgreSQL
 */
@Service
public class PostgreSqlObjectUtilityService extends CommonSqlObjectUtilityService {
    ...
    public SqlObject applyUnaryFunctionToSqlObject(SqlObject sqlObject, VtlExpression parameter, List<VtlExpression> optionalParameter, Operator operator) {
        ...
        if (operator.getValue().equals(Operator.MATCH_CHARACTERS.getValue())) {
            ...
            ce.addExpression(sqlObject).addExpression(((VtlConstantExpression) parameter).getVtlConstant().getValue()).setDisableParens(false);
            return ce;
        }
    ...
    /**
     * Genera una query di unpivot per PostgreSQL
     *
     * @param sqlComponent
    ...
    //getSqLOperator valid for PostgreSql
    ...
    static {
        ...
        map.put("instr", "strpos");
        map.put("match_characters", " similar to ");
    ...
    public SqlObject createCastNull(String domainValue) {
        ...
        if (domainValue.equals(VtlType.STRING.getDomainValue())) {
            castNull = new CustomSql("CAST(NULL AS varchar(100))");
        } else if (domainValue.equals(VtlType.NUMBER.getDomainValue())) {
            castNull = new CustomSql("CAST(NULL AS numeric)");
